...
import { Key, Briefcase, ArrowRight } from 'lucide-react';
...
export default function DashboardGateway() {
  ...
  const selectRole = async (role: 'tenant' | 'landlord' | 'both') => {
    if (profile) {
      // If the user already has a 'both' role, we don't want to overwrite it 
      // with 'tenant' or 'landlord' just because they clicked a card.
  ...
  // If role is 'both', show the standard choice UI
  return (
